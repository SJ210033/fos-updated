...
app.post("/adduser", async (req, res) => {
	...
	db.query(checkemail, [email], (err, emailData) => {
		...
		db.query(checkphoneNumber, [phone], (err, phoneData) => {
			...
			const val = [req.body.userName, email, phone, req.body.password];
...
app.post("/login", (req, res) => {
	...
	const getUserQuery =
		"SELECT * FROM CUSTOMER_REGISTRATION WHERE mail = ? OR phnum = ?";
	db.query(getUserQuery, [email, phoneNumber], (err, result) => {
		...
		//* Create a JWT token
		const token = jwt.sign({ userId: customerID }, secretKey, {
			expiresIn: "1d",
		});
...
app.post("/orderdetails", (req, res) => {
	...
	db.query(sql, [values], (error, results) => {
		if (error) {
			...
			const invoiceValues = [
				[invoiceNumber, orderNumber, customerID, addressID, totalBill],
			];
